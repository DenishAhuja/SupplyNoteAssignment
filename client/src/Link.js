...
const Link = () => {
  ...
  const updateClicks = async (e) => {
    ...
    const data = res.json();
    if (res.status === 202) console.log(data.message);
  ...
  return (
    <div className="">
      ...
      <form action="/shortUrls" method="POST" className="signup-form hh">
        ...
        <input
          ...
          name="fullUrl"
          id="fullUrl"
          onChange={(e) => setShortlinks({ full: e.target.value })}
      ...
      <div className="text-center">
        <a id="link" onClick={updateClicks}>
          {currentlink.short}{" "}
        </a>
        <p> Short Url: {currentlink.clicks}</p>
      </div>
    </div>
  );
};
